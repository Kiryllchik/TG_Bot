...
'''
 КОМАНДА /START
'''
@bot.message_handler(commands=['start']) # Створення команди для бота
def start(message): # виконання дії при введені команди.
    # Повідомлення 1
    id_user(message)
    bot.send_message(chat_id, f'Привіт, {message.from_user.username}❤️! Все ще памʼятаєш про мене? 🥹')
    # Повідомлення 2
...
def daily_photo(message):
    ...
    if 0 <= day_of_week < len(photos):
        ...
        bot.send_message(chat_id, f'Розклад пар у <b>{day_name.upper()}</b>', parse_mode='html')
        bot.send_photo(chat_id, open(photos[day_of_week], 'rb'))

        # Створюємо клавіатуру для вибору іншого дня
        markup = types.InlineKeyboardMarkup()
        for i in range(5):
            markup.add(types.InlineKeyboardButton(calendar[i].upper(), callback_data=calendar[i]))
        bot.send_message(chat_id, "Можливо інший день: ☺️", reply_markup=markup)
    else:
        # Повторюєм цикл оскільки в if він не спрацьовує коли активується else
        markup = types.InlineKeyboardMarkup()
        for i in range(5):
            markup.add(types.InlineKeyboardButton(calendar[i].upper(), callback_data=calendar[i]))
        bot.send_message(chat_id, f'Розклад пар у <b>ПОНЕДІЛОК</b>', parse_mode='html')
...
def set_daily_photo(message, selected_day=None):
    if selected_day in calendar:
        ...
        bot.send_message(chat_id, f'Розклад пар у <b>{calendar[selected_day_index].upper()}</b>', parse_mode='html')
        bot.send_photo(chat_id, open(photos[selected_day_index], 'rb'), reply_markup=markup)
...
def repeat(message):
    ...
    for i in range(1, month):
        button = types.InlineKeyboardButton(f'{str(i)}.10.🕰️', callback_data=str(i))
        row.append(button)
        # Додавання < та >
        if i == 28:
            btn_left = (types.InlineKeyboardButton('<', callback_data='left'))
            btn_right = (types.InlineKeyboardButton('>', callback_data='right'))
            row.append(btn_left)
        elif i == 30:
    ...
    bot.send_message(chat_id, 'Ось які завдання тобі доступні сьогодні: 😘', reply_markup=keyboard)
'''
'''
numbers = []
for i in range(30):
...
def english_form(message):
 id_user(message)
 markup = types.InlineKeyboardMarkup()
 markup.add(types.InlineKeyboardButton('Додати нові слова', callback_data='words'))
 markup.add(types.InlineKeyboardButton('Всі слова', callback_data='learn'))
 markup.add(types.InlineKeyboardButton('Слова додані сьогодні', callback_data='added_today'))
 markup.add(types.InlineKeyboardButton('Повторити слова на сьогодні', callback_data='rep_word'))
 markup.add(types.InlineKeyboardButton('Повторення без запису', callback_data='teach'))
 markup.add(types.InlineKeyboardButton('Вивчені слова', callback_data='complete'))

 bot.send_message(chat_id, 'Виберіть дію для англійської мови', reply_markup=markup)
...
async def time_message():
    global chat_id
    if chat_id:
        await bot.send_message(chat_id, 'Моє серденько❤️, Are you ready for RISE!? \nLet`s start!')
...
def words(message):
    id_user(message)
    bot.send_message(chat_id, 'Введіть слово, яке хочете добавити (або "end", щоб завершити): ')
...
def process_words(message):
    if message.text.lower() == 'end':
        ...
        if existing_word:
            bot.send_message(chat_id, f'Слово "{message.text}" вже є в базі даних.')
        else:
            # Додаємо слово до бази даних
            curr_date()
            next_dat = (current_date + datetime.timedelta(days=1)).strftime("%d/%m/%Y")
            cursor.execute(f'INSERT INTO {idn} (new_words, data_new_words, data_next_rep, amount_of_rep) VALUES (?, ?, ?, ?)', (message.text, current_date.strftime("%d/%m/%Y"), next_dat, 0))
            conn.commit()
            # Повідомлення про успішне додавання слова
            bot.send_message(chat_id, f'Слово "{message.text}" успішно збережено.')
            # Запит на введення наступного слова
        bot.send_message(chat_id, 'Введіть ще одне слово (або "end", щоб завершити): ')
...
def show_words(message):
    ...
    insert(message)
    curr_date()
    cursor.execute(f'SELECT new_words FROM {idn} WHERE complete_words IS NULL AND data_new_words = ?', (current_date.strftime("%d/%m/%Y"),))
    ...
    if words and all(word is not None and isinstance(word, str) for word in words):
        ...
        bot.send_message(chat_id, f'Слова, які ви добавили сьогодні:\n' + '\n'.join(words))
    else:
        # Відправляємо повідомлення, якщо сьогодні не було доданих слів
        bot.send_message(chat_id, 'На жаль, сьогодні ви не додали жодного слова.')
    # Дізнаюсь який сьогодні день з 365
    daysend = current_dato.timetuple().tm_yday
    goal = 300
    button = types.InlineKeyboardMarkup()
    button.add(types.InlineKeyboardButton('Повторити слова', callback_data='teach'))


    cursor.execute(f'SELECT complete_words FROM {idn} WHERE new_words IS NULL')
    complete = cursor.fetchall()
    bot.send_message(chat_id, '///Можливість створення власних цілей, у розробці///')
    words = [word[0] for word in complete]
    bot.send_message(chat_id, 'Твоя ціль вчити 5 слів у день. Тоді ти зможеш до кінця року вивчити більше ніж 300 слів😍😘\n Приблизно за 60 днів🥳 \n Слова треба повторювати і ти це будеш робити 5 разів для слова, а також будеш придумувати до нього речення.')
    bot.send_message(chat_id, f'До кінця року <b>{str(365 - int(daysend))}</b>', parse_mode='html')

    bot.send_message(chat_id, f'Кількість вивчених слів: <b>{str(len(words))}</b>', parse_mode='html')
    bot.send_message(chat_id, f'Ще треба вивчити слів: <b>{str(int(goal)-int(len(words)))}</b>', parse_mode='html', reply_markup=button)
    cursor.close()
    conn.close()
...
def new_words(message):
    id_user(message)
    insert(message)
    cursor.execute(f'SELECT new_words, data_new_words, data_next_rep, amount_of_rep FROM {idn}')
    complete = cursor.fetchall()
    ...
    bot.send_message(chat_id, f'Слова, які ти вчиш:\n{message_text}', parse_mode='html')
...
def repeat_words(message):
    ...
    selected_words = []
    for row in complete:
        db_date = datetime.datetime.strptime(row[1], "%d/%m/%Y")  # Перетворення рядка у datetime
    ...
    if complete:
        ...
        bot.send_message(chat_id, 'Ось слова, які ти можеш повторити сьогодні:\n' + '\n'.join([f'<b>{word}</b>' for word in selected_words]), parse_mode='html', reply_markup=button)
        bot.send_message(chat_id, 'Памʼятай, у бота тобі нічого писати не треба (можливо це тимчасово 🤷‍♂️). Я надаю тобі підказки, яким чином буде ефективно повторити слово, яке є на черзі. \nКоли ти їх виконаєш і тільки тоді, ти можеш вказати, що успішно повторив слово (тобто нажати True❤️). \nЯкщо ж ти слово не згадав, дотримуйся підказок для повторення, і вибери False💔.')    

    else:
        bot.send_message(chat_id, 'Сьогодні немає слів для вивчення :( \nПовертайся завтра або переглянь усі слова, які ти вивчаєш :)')    
...
def start_repeat_words(message):
    ...
    if complete:
      ...
      bot.send_message(chat_id, 'Інструкція: Якщо ти памʼятаєш переклад цього слова напиши 4 речення з ним. (///beta_text!///)')
      foo(message)
    else:
      bot.send_message(chat_id, 'Ти ще не додав жодного слова, яке можна почати вчити.')
...
def foo(message): 
    ...
    if len(worder) == 0:
        ...
        bot.send_message(chat_id, f'Чи памʼятаєш ти це слово? \n\n<b>{one_word}</b>', parse_mode='html', reply_markup=button)
        if message.text.lower() == 'end':
          bot.send_message(chat_id, f'Молодець! Ти чудово постарався. Сьогодні ти повторив {str(len(wordso))} слів🥳 \n{str(len(selected_words) - len(stat_false))} слова ти згадав та {str(len(stat_false))} не згадав. \nЧекаю тебе завтра або в будь-який інший час❤️')
          del one_word
          del stat_false
          return
    else:
...
def complete_word(message):
    ...
    words = [word[0] for word in complete]
    if complete:
        bot.send_message(chat_id, 'Ось слова, які ти ВИВЧИВ: \n' + '\n'.join(words))
    else:
...
def call_message(callback):
    if callback.data == 'start_rep_word':
      ...
      amount_rep[0] += 1
      if amount_rep[0] == 7:
        cursor.execute(f'UPDATE {idn} SET data_next_rep = NULL, complete_words = 1 WHERE new_words = {one_word}')
        bot.send_message(chat_id, '🎉Вітаю, ти вивчив це слово 🥳🥇')
        conn.commit()
      foo(callback.message)
    elif callback.data == 'false':
      bot.send_message(chat_id, 'Що ж, чим більше ти помиляєшся, тим кращим буде кінцевий результат.')
      ...
      daily_photo(callback.message)
    elif callback.data == 'repeat':
      repeat(callback.message)
    elif callback.data == 'teach':
      teach(info_user)
    elif callback.data in numbers:
      repeat_data(callback.message, callback.data)
...
def iii(message):
    if message.text == 'end':
        ...
        insert(message)
        cursor.execute(f'SELECT complete_words FROM {idn} WHERE complete_words = ?', (message.text,))
        existing_word = cursor.fetchone()

        if existing_word:
            bot.send_message(chat_id, f'Слово "{message.text}" вже є в базі даних.')
        else:
            cursor.execute(f'INSERT INTO {idn} (complete_words, data_new_words) VALUES (?, ?)', (message.text,))
            conn.commit()
...
def repeat_data(message, data):
    ...
    for i in range(3):
        keyboard.add(types.InlineKeyboardButton(named[i], callback_data=named[i].lower()))
    bot.send_message(chat_id, f'Вибери тему за <b>{str(data)}.10</b>: ', reply_markup=keyboard, parse_mode='html')
